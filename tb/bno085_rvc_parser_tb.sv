module bno085_rvc_parser_tb;
    logic clk=0, reset=1, byte_valid=0;
    logic [7:0] byte_data=0;
    logic signed [15:0] yaw_cd,pitch_cd,roll_cd,ax,ay,az;
    logic [7:0] seq;
    logic valid, checksum_error, sensor_timeout, saw_valid=0;
    bno085_rvc_parser #(.CLK_HZ(1000), .TIMEOUT_MS(100)) dut (
    ...
    always #5 clk=~clk;
    always @(posedge clk) if (valid) saw_valid <= 1;
    ...
    initial begin
        ...
        @(posedge clk); #1;
        if (!saw_valid || seq!=8'h07 || yaw_cd!=16'sh1234 || pitch_cd!=-2 || roll_cd!=-100 ||
            ax!=10 || ay!=-10 || az!=1000 || sensor_timeout)
            $fatal(1, "RVC parser did not decode expected values");
        $display("PASS: bno085_rvc_parser_tb");
        $finish;
    end
endmodule
