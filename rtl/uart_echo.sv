...
module uart_rx #(
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin rx_meta <= 1'b1; rx_sync <= 1'b1; end
...
module uart_tx #(
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            Tx <= 1'b1; busy <= 1'b0; frame <= 10'h3ff; bit_index <= 0; count <= 0;
        end else if (!busy) begin
            Tx <= 1'b1;
            if (start) begin
                frame <= {1'b1, data, 1'b0}; bit_index <= 0;
                count <= CLKS_PER_BIT - 1; Tx <= 1'b0; busy <= 1'b1;
            end
        end else if (count == 0) begin
            if (bit_index == 9) begin Tx <= 1'b1; busy <= 1'b0; end
            else begin
...
// Legacy echo remains available for the original UART smoke test.
module uart_echo #(
    parameter integer CLK_HZ = 12_000_000,
    parameter integer BAUD   = 115_200
) (
    ...
    uart_tx #(.CLK_HZ(CLK_HZ), .BAUD(BAUD)) transmitter (
        .clk(clk), .reset(reset), .data(last_byte), .start(tx_start), .Tx(Tx), .busy(tx_busy));

    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin last_byte <= 0; rx_seen <= 1'b0; tx_start <= 1'b0; end
        else begin
            tx_start <= 1'b0;
            if (rx_valid) begin
                last_byte <= rx_data; rx_seen <= 1'b1;
...
// Ordinary bytes are echoed. 0xF0 is reserved for a one-byte status reply.
module uart_command_link #(
    parameter integer CLK_HZ = 12_000_000,
    parameter integer BAUD   = 115_200
) (
    input logic clk, reset, Rx,
    ...
    input logic [7:0] reply_data,
    input logic reply_start,
    output logic [7:0] last_byte,
    output logic rx_seen, tx_busy
);
    logic [7:0] tx_data;
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            tx_data <= 0; tx_start <= 1'b0; last_byte <= 0; rx_seen <= 1'b0;
        end else begin
            tx_start <= 1'b0;
            if (reply_start && !tx_busy) begin
                tx_data <= reply_data; tx_start <= 1'b1;
            end else if (rx_valid) begin
                last_byte <= rx_data; rx_seen <= 1'b1;
                if (rx_data != 8'hF0 && !tx_busy) begin
                    tx_data <= rx_data; tx_start <= 1'b1;
                end
            end
        end
    end
