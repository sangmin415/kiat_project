module top_bno085_pc_servo (
    input logic hwclk, input logic bno_rxc, input logic pc_rxc,
    ...
    bno085_rvc_parser_cpu sensor(
        ...
        .accel_x_mg(ax),.accel_y_mg(ay),.accel_z_mg(az),.sample_seq(sample_seq),
        .sample_valid(sample_valid),.checksum_error(checksum_error),.sensor_timeout(sensor_timeout));
    rvc_uart_forwarder forwarder(
        .clk(hwclk),.reset(reset),.rx_data(bno_byte),.rx_valid(bno_valid),.Tx(Tx),.overflow(forward_overflow));
    ...
    function automatic [17:0] clamp_pwm(input [15:0] ticks);
        if(ticks<PWM_MIN) clamp_pwm=PWM_MIN;
        else if(ticks>PWM_MAX) clamp_pwm=PWM_MAX;
        else clamp_pwm=ticks;
    endfunction
    ...
    always_ff @(posedge hwclk)
        if(reset||frame_count==FRAME_TICKS-1) frame_count<=0; else frame_count<=frame_count+1'b1;
    assign servo_r0=frame_count<safe_r0;
    assign servo_r1=frame_count<safe_r1;
    assign servo_r2=frame_count<safe_r2;
